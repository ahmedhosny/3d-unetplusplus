...
from decoder_blocks import Transpose2D_block, Upsample2D_block
        ...
        for i in range(n_upsample_blocks-j):
            ...
            if i == 0 and j < n_upsample_blocks-1 and len(skip_connection_layers) < n_upsample_blocks:
                ...
                if downterm[i+1] is not None:
                    interm[(n_upsample_blocks+1)*i+j+1] = up_block(downterm[i+1], decoder_filters[n_upsample_blocks-i-2], i+1, j+1, upsample_rate=upsample_rate, skip=interm[(n_upsample_blocks+1)*i+j],
                    use_batchnorm=use_batchnorm)
                else:
                    interm[(n_upsample_blocks+1)*i+j+1] = None
            else:
                interm[(n_upsample_blocks+1)*i+j+1] = up_block(interm[(n_upsample_blocks+1)*(i+1)+j], decoder_filters[n_upsample_blocks-i-2], i+1, j+1, upsample_rate=upsample_rate, skip=interm[(n_upsample_blocks+1)*i : (n_upsample_blocks+1)*i+j+1],
                use_batchnorm=use_batchnorm)
