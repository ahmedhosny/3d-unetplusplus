...
def get_bbox_metrics(tensor, flag):
    ...
    #
    Z_center = tf.subtract(Z_max,Z_min)/2 + tf.cast(Z_min, tf.float64)
    Y_center = tf.subtract(Y_max,Y_min)/2 + tf.cast(Y_min, tf.float64)
    X_center = tf.subtract(X_max,X_min)/2 + tf.cast(X_min, tf.float64)
    #
    return Z_center, Y_center, X_center

def bbox_distance_loss_1(y_true, y_pred):
    '''
...
def bbox_distance_loss(y_true, y_pred):
    ...
    X = tf.reduce_sum(y_pred, axis=(0,1))
    X = K.mean(X)
    # X = tf.cast(X, tf.bool)
    # out1 = tf.where(X)[50][0]
