...
def show_axis(data, mask, axis, index, slice, bbox, show_bbox, mask_count):
    ...
    if show_bbox:
        ...
        linewidth=line_width, edgecolor=mask_cmap[mask_count], facecolor='none')
        axis.add_patch(rect)
...
def plot_figure(dataset, patient_id, data_arr, mask_arr_list, mask_list_names, com_gt, com_pred, bbox_list, show_bbox, output_dir, distance, dice):
    ...
    #
    title =  "{}_{}\ndistance: {}mm\ngt bbox center: {} pred bbox center: {}\ndice: {}".format(dataset, patient_id, round(distance, 3), com_gt, com_pred, round(dice, 3))
    name = "{}_{}".format(dataset, patient_id)
    fig.suptitle(title, fontsize=16)

    axial_idx = [bbox[0], bbox[0] + int((com_gt[0] - bbox[0])//2.), com_gt[0], bbox[1] - int((bbox[1] - com_gt[0])//2.) ,bbox[1]]
    coronal_idx = [bbox[2], bbox[2] + int((com_gt[1] - bbox[2])//2.), com_gt[1], bbox[3] - int((bbox[3] - com_gt[1])//2.) ,bbox[3]]
    sagittal_idx = [bbox[4], bbox[4] + int((com_gt[2] - bbox[4])//2.), com_gt[2], bbox[5] - int((bbox[5] - com_gt[2])//2.) ,bbox[5]]
...
def plot_legend(ax, mask_list_names):
    ...
    for i, mask_name in enumerate(mask_list_names):
        if mask_name is not None:
            rectangles[mask_name] = patches.Rectangle((-70, i*50), 50, 30, clip_on=False, facecolor=mask_cmap[i], edgecolor="k", linewidth=3)
...
def plot_images(dataset, patient_id, data_arr, gt_arr, pred_arr, output_dir, bbox_flag, bbox_metrics, dice):
    ...
    try:
        ...
        # plot
        plot_figure(dataset, patient_id, data_arr, mask_arr_list, mask_list_names, com_gt, com_pred, [gt_bbox, pred_bbox], bbox_flag, output_dir, bbox_metrics["distance"], dice)
